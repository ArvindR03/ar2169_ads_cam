...
def terminate_connection_pymysql(conn):
    """
    Given a connection with the Jupyter %sql extension, terminate the connection safely.
    :param conn
    """
    conn.close()
...
def preprocess_historical_election_df(df):
    ...
    england_constituencies = ['North West',
       'Yorkshire and the Humber', 'South West', 'London', 'Eastern',
       'South East', 'West Midlands', 'University', 'North East',
       'East Midlands', 'Yorkshire & The Humber',
       'Yorkshire and The Humber']
    df = df[df['country/region'].isin(england_constituencies)]
...
def return_combined_area_gdf_constituencies(geometries_dict, ids_df, epsg=4326, on="id"):
    ...
    """
    # step 1: convert this dict to a gdf
...
# DONE
def process_constituency_name(name):
    """
    Given a constituency name, preprocess and return it.
    :param name
    """
    # remove puncuation
    name = name.lower().translate(str.maketrans('', '', string.punctuation))

    # remove mid, north, south, east, west, punctuation, the, and, of
    words_to_remove = set(['mid', 'north', 'south', 'east', 'west', 'punctuation', 'the', 'and', 'of'])
    name = '+'.join([w for w in name.split() if w not in words_to_remove])
    return name

# DONE
def get_coords_from_nominatim_api(place_name):
    """
    Helper function.
    Query the Nominatim api given a place name (as in the API docs).
    :param place_name
    """
    query = f"https://nominatim.openstreetmap.org/search?q={place_name},+england&format=json&type=administrative"
    headers = {
        "User-Agent": "ads-assessment-ar2169/1.0"
    } # without this it returns 403, the API must be able to identify the request
    ...
    if len(response) == 0:
        ...
        # returns a bounding box, pick from this
        box = response[0]["boundingbox"]
        return Point(box[3], box[0])
...
def add_lookup_for_census_into_constituencies_table(constituencies_df, census_df, lookup_colname="lookup_census"):
    ...
    # no need, we can add directly later

    # step 2: calculate values for the column
    lookups = []
    for _, constit_row in constituencies_df.iterrows():
        vals = []
        name = constit_row["geom_name"]
        for _, census_row in census_df.iterrows():
            census_name = census_row['parliamentary constituency 1983 revision']
            census_idx = census_row["mnemonic"]
            vals.append((census_idx, fuzz.ratio(name, census_name))) # tuple of idx and ratio
        
        # find max val, and append to lookups corresp census id
        lookups.append(max(vals, key=lambda x: x[1])[0])
    
    # step 3: append values to new col and return
    constituencies_df[lookup_colname] = lookups
    return constituencies_df

# DONE
def get_response_variable_for_boundary_set(election_historical_df, boundary_set='1974-1979'):
    """
    ...
    df.fillna(0.0)
    df.replace(to_replace=-1.0, value=0.0)
    ...
    # step 4: join back the names of constituencies and return, we do the groupby stuff to avoid dupe rows
    return pd.merge(result_df, df[["constituency_id", "constituency_name"]], how="left", on="constituency_id").groupby(["constituency_id"])[["constituency_name", "response_variable"]].first().reset_index()

# DONE
def augment_census_with_response_vars(census_df, response_vars_df):
    """
    Again we use string fuzzymatching to be able to resolve spelling differences in names.
    :param census_df
    :param response_vars_df
    """
    response_vars_to_add_to_census = []
    for _, census_row in census_df.iterrows():
        const_name = census_row["parliamentary constituency 1983 revision"]
        name_ratios = []
        for _, response_row in response_vars_df.iterrows():
            response_name = response_row["constituency_name"]
            response_var = response_row["response_variable"]
            name_ratios.append((response_var, fuzz.ratio(const_name, response_name)))
        response_vars_to_add_to_census.append(max(name_ratios, key=lambda x:x[1])[0])
    
    census_df["response_variable"] = response_vars_to_add_to_census
    return census_df
...
def rename_cols_to_fit_mariadb_limit(df):
    ...
    :param df
    """
    df.columns = [i[:61] for i in df.columns]
    return df
